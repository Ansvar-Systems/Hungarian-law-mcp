#!/usr/bin/env tsx
/**
 * Paid-tier database builder for Hungarian Law MCP server.
 *
...
const DEFAULT_DB_PATH = path.resolve(__dirname, '../../../Hungarian-law-mcp/data/database.db');
...
CREATE TABLE IF NOT EXISTS case_law (
  ...
  source TEXT NOT NULL DEFAULT 'birosag_hu',
...
CREATE TABLE IF NOT EXISTS preparatory_works (
  ...
  committee TEXT,
  proposer TEXT,
  source TEXT NOT NULL DEFAULT 'parlament_hu',
  inserted_at TEXT DEFAULT (datetime('now'))
);
...
function buildPaidTier(): void {
  ...
  console.log('Building paid-tier extensions for Hungarian Law MCP...\n');
  ...
  const updateMeta = db.transaction(() => {
    ...
    upsertMeta.run('jurisdiction', 'HU');
    upsertMeta.run('paid_tables', 'case_law,case_law_full,preparatory_works,preparatory_works_full,agency_guidance');
    upsertMeta.run('source_case_law', 'birosag.hu,huncourt dataset');
    upsertMeta.run('source_prep_works', 'parlament.hu');
    upsertMeta.run('source_agency', 'naih.hu,gvh.hu');
  });
  updateMeta();
