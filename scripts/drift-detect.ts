#!/usr/bin/env tsx
/**
 * Drift detection for Hungarian Law MCP.
 *
 * Checks if upstream njt.hu content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Hungarian Law MCP — Drift Detection');
  console.log('=====================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'Hungarian-Law-MCP/1.0 drift-detect' },
      });
